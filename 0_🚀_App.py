...
def render_dashboard():
    ...
    def calcola_fatturato_reale(row):
        ...
        try:
            ...
            for item in incassi:
                # --- PUNTO CRUCIALE: RISOLUZIONE ANNIDAMENTO ---
                # Il dato potrebbe essere {"Stato":...} oppure {"0": {"Stato":...}}
                dati_reali = item
                
                # Se l'item è un dizionario ma non ha le chiavi dirette, cerchiamo nei valori
                if isinstance(item, dict) and "Stato" not in item:
                    # Controlliamo se c'è un dizionario annidato (es. sotto la chiave "0")
                    for k, v in item.items():
                        if isinstance(v, dict) and "Stato" in v:
                            dati_reali = v
                            break # Trovato, usiamo questo
                
                # --- ORA LEGGIAMO I DATI VERI ---
                stato = str(dati_reali.get("Stato", "")).lower().strip()
                
                # Cerca "fatturato" ignorando maiuscole/minuscole
                if "fatturato" in stato:
                    raw_importo = dati_reali.get("Importo netto €", 0)
                    
                    # Pulizia numero (1.000,00 -> 1000.0)
    ...
    # Applica la nuova funzione "intelligente"
    ...
    if not df.empty:
        ...
        df_view["Fatturato"] = df_view["Fatturato_Reale"] # Mostriamo il valore corretto

        cols_show = ["Seleziona", "Codice", "Cliente", "Nome Commessa", "Settore", "Totale Commessa", "Fatturato"]
        actual_cols = [c for c in cols_show if c in df_view.columns]
        ...
        if not rows_del.empty:
            if st.button(f"🗑️ ELIMINA {len(rows_del)} COMMESSE", type="primary"):
